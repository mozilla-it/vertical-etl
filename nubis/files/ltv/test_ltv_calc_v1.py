...
def main():
    ...
    distinct_clients_query = ("SELECT distinct client_id from ut_clients_search_history where sap>0 order by client_id limit 150000;")
    # client_hist_query = ("select hist_country.client_id, hist_country.submission_date_s3, hist_country.sap, IFNULL(rev.rev_per_search,.005) from ( (select hist.*,deets.country from (select * from ut_clients_search_history where sap>0 and client_id in ( {client_list} )) hist left join (select client_id, country from ut_clients_daily_details) deets on hist.client_id=deets.client_id) hist_country left join ut_country_revenue rev on hist_country.country=rev.country_code);")
    ...
    fo = 'fileout.csv'
    with open(fo, 'a') as f:
      for group in grouper(client_ids, 100000):
         print(process.memory_info().rss)

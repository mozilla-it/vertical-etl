...
def main():
    ...
    input_fn = max(glob.glob(input_dir + 'mobile_active_counts_*'), key=os.path.getctime)
    if not input_fn:
    ...
    load_into_vertica(output_fn,vertica_table_name,delimiter='\t',field_order = field_order)
